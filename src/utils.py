"""
Credit goes to:
https://aquaulb.github.io/book_solving_pde_mooc/solving_pde_mooc/notebooks/04_PartialDifferentialEquations/04_03_Diffusion_Explicit.html
"""
...
class Stepper:
    ...
    def rk4_step(self, T_t: np.ndarray, *args) -> np.ndarray:
        ...
        T_t is the values at the current time step, ergo temperature
        values (alternatively: t0)
...
def analytical_1d_heat_eq(
    ...
    # Without transient (homogeneous IC: u(x,0) = 0)
    solution = (
        heat_source
        / (alpha * np.pi**2)
        * (1 - np.exp(-alpha * np.pi**2 * time))
