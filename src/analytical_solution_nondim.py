...
if __name__ == "__main__":
    #
    # * Introductory example, factor out alpha and heat_source
    #
    char_temp = HEAT_SOURCE_INTENSITY / ALPHA
    char_time = 1 / ALPHA
    ...
    plt.tight_layout()
    plt.savefig(
        "src/plots/non-dimensionalized_analytical_solution_alpha=1.png", dpi=200
    )
    # plt.show()
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(
        "src/plots/non-dimensionalized_analytical_solution_alpha=0.1.png", dpi=200
    )
    # plt.show()
    plt.close()
    ...
    for idx, time in enumerate(np.linspace(0, 30, NUM_OF_TIME_STEPS)):
        ...
        if idx % 200 == 0:
            plt.plot(
                X_DOMAIN / max(X_DOMAIN), temperature * char_temp, label=f"t={time:.2f}"
            )
    ...
    plt.tight_layout()
    plt.savefig(
        "src/plots/non-dimensionalized_analytical_solution_L=2_alpha=0.1.png", dpi=200
    )
    # plt.show()
    plt.close()
