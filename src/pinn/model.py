...
class HeatEq1DPINNRegressor(pl.LightningModule):
    ...
    def configure_linears(self) -> torch.nn.modules.container.ModuleList:
        ...
        hidden_layers_list = np.repeat(
            self.hparams.size_hidden_layers, self.hparams.num_hidden_layers + 1
        )
        layers_list = np.array(
            [self.hparams.in_features, *hidden_layers_list, self.hparams.out_features]
        )

        linears = torch.nn.ModuleList(
            [
                torch.nn.Linear(layers_list[i], layers_list[i + 1])
    ...
    def configure_optimizers(self) -> Dict:
        ...
        elif (self.hparams.optimizer == torch.optim.Adam) or (
    ...
    def training_step(self, train_batch, batch_idx) -> float:
        ...
        loss_BC = self.pinn_losses.loss_function_IC_BC(
            y_pred=y_pred_BC, y_train=y_train_BC
        )

        y_pred_IC = self.forward(x_train_IC)  # same as torch.cat
        loss_IC = self.pinn_losses.loss_function_IC_BC(
            y_pred=y_pred_IC, y_train=y_train_IC
        )
        ...
        loss_PDE = (
            self.pinn_losses.loss_function_PDE(
                u_t,
                u_xx,
                x_train_x,
            )
        ...
        loss = (
            loss_PDE
            + loss_BC * self.hparams.loss_BC_param
            + loss_IC * self.hparams.loss_IC_param
        )
        ...
        if self.current_epoch % 100 == 0:
            print(
                "Iter %d, Loss: %.5e, Loss_IC: %.5e, Loss_BC: %.5e, Loss_PDE: %.5e"
                % (
                    self.current_epoch,
                    loss.item(),
                    loss_IC.item(),
