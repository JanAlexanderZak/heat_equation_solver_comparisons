...
class HeatEq1DPINNRegressor(pl.LightningModule):
    ...
    def training_step(self, train_batch, batch_idx) -> float:
        ...
        x_train_BC_t = torch.tensor(x_train_BC[:, 1:2], requires_grad=True).float()

        y_pred_BC = self.net(x_train_BC_x, x_train_BC_t)
        ...
        if self.current_epoch % 100 == 0:
            print(
                "Iter %d, Loss: %.5e, Loss_BC: %.5e, Loss_PDE: %.5e"
                % (self.current_epoch, loss.item(), loss_BC.item(), loss_PDE.item())
            )
        return loss
