...
from constants import ALPHA, THERMAL_DIFFUSIVITY
...
if __name__ == "__main__":
    ...
    # Time (depends on thermal_diffusivity)
    REFERENCE_TIME = 10
    ...
    for time_step in TIME_STEPS:
        ...
        if time_step % 200 == 0:
            plt.plot(
                X_DOMAIN, T[time_step] * char_temp, label=f"t={time_step * dt:.2f}"
            )
    ...
    for time_step in TIME_STEPS:
        ...
        if time_step % 550 == 0:
            plt.plot(
                X_DOMAIN / max(X_DOMAIN),
                T[time_step] * char_temp,
                label=f"t={time_step * dt:.2f}",
            )
